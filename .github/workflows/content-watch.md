...
2. Open `<details>` sections in catalogue area order, then `Uncategorized`; sort each section by topic, retaining discovery order within a topic. Use a two-column `Item | Take` table. Use 🛠️, 🗣️, 📚, 🧠, 💭, and ❓ for `tech`, `comm`, `learn`, `mem`, `think`, and `uncategorized`. Every Item cell is exactly one Markdown link, all on one line, with no space or newline anywhere between the closing `]` and the opening `(`: `[title](url)`. Correct: `[GPT-5 system card](https://example.com/gpt-5)`. Incorrect (never do this): `[GPT-5 system card]` followed by `(https://example.com/gpt-5)` on its own line, `[title] (url)` with a space, or doubled parens like `((url))`. If a title itself contains parentheses, keep the whole title inside the brackets and the URL inside its own single set of parens — e.g. `[models may behave differently (a tirade)](https://example.com/x)`, never letting the title's `(...)` merge with or replace the link's `(url)`. Link RSS/page titles to their canonical links, and for GitHub items link titles to Learn when available as `[title](learnUrl) ([diff](commitLink))`, otherwise to the commit URL — both links stay on one line with no space before either `(`. Replace uninformative commit titles with a diff-grounded label. The Take cell is exactly two lines: `**{Topic}** · <take><br>\`<concept or —>\` · <fit> · <published YYYY-MM-DD or —>`, with no other line breaks. Always include the `<published YYYY-MM-DD or —>` segment — never drop or shorten it.
